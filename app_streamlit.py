#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# EEG-Auswertung – Datei-Upload + Auswertung + PNG-Rendering (Plotly→Kaleido, Fallback Matplotlib)
...
# ---------- Matplotlib-Fallback für PNG (kein Chrome nötig) ----------
import matplotlib
matplotlib.use("Agg")
...
def render_png_matplotlib(df, kind="stress_relax", smooth=5, outpath="render.png"):
    ...
    if kind == "stress_relax":
        ...
        ax.plot(x, d["stress_trend"], color=(0.86,0.27,0.27), linewidth=3, marker="o", label="Stress (Trend)")

        ax.fill_between(x, d["relax_trend"]-d["relax_std"], d["relax_trend"]+d["relax_std"],
                        alpha=0.20, color=(0.27,0.67,0.27))
        ax.plot(x, d["relax_trend"], color=(0.27,0.67,0.27), linewidth=3, marker="o", label="Entspannung (Trend)")

        ax.set_ylabel("Index")
        ax.set_title("Stress- und Entspannungs-Trend mit Schattierung")
        ax.legend(loc="best")
        ...
        ax.plot(x, d["delta_trend"],  linewidth=3, label="Delta")
        ax.plot(x, d["theta_trend"],  linewidth=3, label="Theta")
        ax.plot(x, d["alpha_trend"],  linewidth=3, label="Alpha")
        ax.plot(x, d["beta_trend"],   linewidth=3, label="Beta")
        ax.plot(x, d["gamma_trend"],  linewidth=3, label="Gamma")
        ax.set_ylim(0, 1)
...
def load_session_relatives(csv_path):
    """Erwartet Spalten Delta_* / Theta_* / Alpha_* / Beta_* / Gamma_* oder einzelne Delta/…"""
    try:
        df = pd.read_csv(csv_path, low_memory=False)
    except Exception:
        return None
    bands = ["Delta", "Theta", "Alpha", "Beta", "Gamma"]
    cols = {b: [c for c in df.columns if str(c).startswith(f"{b}_")] or ([b] if b in df.columns else [])
            for b in bands}
...
def make_beauty_figure(df, kind="stress_relax", smooth=5):
    """Schöne Plotly-Darstellung mit Schattierung."""
    x = df["date_str"]
    if kind == "stress_relax":
        d = df.copy()
        d["stress_trend"] = d["stress"].rolling(smooth, center=True, min_periods=1).mean()
...
if st.button("Auswertung starten"):
    ...
    if not selected_csvs:
        ...
        for i, cp in enumerate(sorted(selected_csvs), start=1):
            ...
            stat.text(f"Processed {i}/{total}: {os.path.basename(cp)}")
        ...
        if not df.empty:
            ...
            # Ergebnis für Export persistent speichern
...
# ---------- 4) Export (PNG) – unabhängig vom Auswertungs-Button ----------
st.subheader("Export")

# aus Session-State lesen; falls leer und 'df' existiert, fallback
df_render = st.session_state.get("df_summary", pd.DataFrame())
if (df_render is None or df_render.empty) and "df" in locals() and isinstance(df, pd.DataFrame) and not df.empty:
    df_render = df.copy()
    st.session_state["df_summary"] = df_render
...
if df_render is None or df_render.empty:
    ...
    if render_btn:
        ...
        try:
            # Plotly/Kaleido versuchen (falls verfügbar)
            if not HAS_KALEIDO:
            ...
            # Fallback: Matplotlib ohne Chrome
            png_path = render_png_matplotlib(df_render, kind=kind, smooth=smooth, outpath=png_path)
...
with st.expander("Debug / Wartung", expanded=False):
    if st.button("Arbeitsordner leeren"):
        ...
        st.session_state.pop("render_path", None)
