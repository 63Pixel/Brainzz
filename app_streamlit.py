...
def preprocess_csv_if_raw(csv_path, out_tmp_dir, fs=250.0):
    # Wenn schon Bandspalten vorhanden, nichts tun
    try:
...
def resolve_selected_to_csvs(selected_paths, workdir):
    """Wandelt gewählte Remote-Pfade (ZIP/SIP/CSV) in lokale CSV-Pfade um."""
    csvs, seen = [], set()
    # Sicherheit: erst (nochmal) entpacken
    ...
    for remote in selected_paths:
        ...
        if ext.lower() in (".zip", ".sip"):
            # CSVs aus dem entpackten Archiv
...
# FTP
    ...
    st.info("FTP ist unverschlüsselt. Für SFTP die SFTP-Option nutzen.")
...
# Dropbox
    ...
    if not HAS_DROPBOX:
        ...
        if not token:
            ...
            # Tabelle nur mit Beschreibung "Datum · Größe"
            rows=[]
            for f in files:
                ...
                rows.append({"description": desc, "path_lower": f.path_lower})
            df_files = pd.DataFrame(rows)

            # AgGrid mit Checkboxen; Fallback: Checkbox-Liste
            try:
                ...
                if df_files.empty:
                    ...
                    gb.configure_grid_options(
                        ...
                        rowMultiSelectWithClick=True, pagination=True,
                        paginationAutoPageSize=False, paginationPageSize=30
                    )
                    grid = AgGrid(
                        df_files[["description","path_lower"]],
                        gridOptions=gb.build(), height=360,
                ...
                if df_files.empty:
                    ...
                    for i,row in df_files.iterrows():
                        key = f"fb_{i}"
                        chk = st.checkbox(row["description"], key=key, value=(key in st.session_state["fb_sel"]))
                        if chk: st.session_state["fb_sel"].add(key)
            ...
            if st.button("Herunterladen (Auswahl)"):
                if not selected_paths:
                    ...
                    for remote in selected_paths:
                        try:
                            download_dropbox_file(token, remote, workdir); ok += 1
                        except Exception as e:
                            st.error(f"Download fehlgeschlagen: {remote} — {e}")
                    recursively_extract_archives(workdir)
                    st.success(f"{ok} Datei(en) heruntergeladen und extrahiert.")
...
if st.button("Auswertung starten"):
    ...
    # bevorzugt Auswahl aus Dropbox → echte CSVs
    selected_csvs = []
    if 'selected_paths' in locals() and selected_paths:
